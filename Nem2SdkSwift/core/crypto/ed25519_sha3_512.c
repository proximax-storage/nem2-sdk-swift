...
void ed25519_sha3_create_keypair(unsigned char *public_key, unsigned char *private_key, const unsigned char *seed) {
    ...
    rhash_sha3_update(&ctx, seed, 32);
    rhash_sha3_final(&ctx, private_key);
...
void ed25519_sha3_sign(unsigned char *signature, const unsigned char *message, size_t message_len, const unsigned char *public_key, const unsigned char *private_key) {
    ...
    rhash_sha3_512_init(&hash);
    rhash_sha3_update(&hash, private_key + 32, 32);
    rhash_sha3_update(&hash, message, message_len);
    rhash_sha3_final(&hash, r);
    ...
    rhash_sha3_512_init(&hash);
    rhash_sha3_update(&hash, signature, 32);
    rhash_sha3_update(&hash, public_key, 32);
    rhash_sha3_update(&hash, message, message_len);
    rhash_sha3_final(&hash, hram);
